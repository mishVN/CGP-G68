    ...
    try {
        ...
        if (userId != -1) {
            ...
            if ("returnOrder".equals(action) && orderId != null) {
                PreparedStatement updateStmt = conn.prepareStatement("UPDATE oders SET status='Return' WHERE oder_id=?");
            ...
            PreparedStatement ordersStmt = conn.prepareStatement(
                "SELECT * FROM oders WHERE customer_id = ? AND status IN ('pending', 'packed', 'on delivery')"
            );
